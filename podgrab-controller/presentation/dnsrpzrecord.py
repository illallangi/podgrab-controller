...
CONTROLLER="presentation-dnsrpzrecord"
...
async def presentation_dnsrpzrecord_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=presentation_dnsrpzrecord_idx.__name__,
)
async def presentation_dnsrpzrecord_probe(
    presentation_dnsrpzrecord_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o
            for o in presentation_dnsrpzrecord_idx[namespace]
        }
        for namespace in presentation_dnsrpzrecord_idx
    }
...
async def presentation_dnsrpzrecord(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    body = (
        load(
            env.get_template("dnsrpzrecord.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                    LABEL_INSTANCE,
